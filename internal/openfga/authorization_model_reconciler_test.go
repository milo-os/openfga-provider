...
func TestAuthorizationModelReconciler_Deduplication(t *testing.T) {
			...
			return &openfgav1.ReadAuthorizationModelResponse{
				AuthorizationModel: &openfgav1.AuthorizationModel{
					...
					TypeDefinitions: []*openfgav1.TypeDefinition{
						...
						{Type: TypeRoot},
						{Type: "test.datumapis.com/TestResource"},
					},
				},
			}, nil
		},
	...
	// First call should query OpenFGA
	err := reconciler.ReconcileAuthorizationModel(context.Background(), prs)
	require.NoError(t, err)
	assert.Equal(t, 1, readCount, "first reconciliation should query OpenFGA")

	// Second call with identical PRs should skip OpenFGA queries
	err = reconciler.ReconcileAuthorizationModel(context.Background(), prs)
	require.NoError(t, err)
	assert.Equal(t, 1, readCount, "second reconciliation with identical inputs should be deduplicated")

	// Third call with updated Generation should query OpenFGA again
	prs[0].Generation = 2
	err = reconciler.ReconcileAuthorizationModel(context.Background(), prs)
	require.NoError(t, err)
	assert.Equal(t, 2, readCount, "reconciliation with modified generation should query OpenFGA again")
}
